//
// Created by biba_bo on 2020-08-07.
//
...
#define CROPPED_WIDTH           200
#define CROPPED_HEIGHT          160
#define CROPPED_X               220
#define CROPPED_Y               120
...
static GstPadProbeReturn cb_have_data (GstPad *pad, GstPadProbeInfo *info, gpointer user_data) {
    ...
    if (gst_buffer_map (buffer, &map, GST_MAP_WRITE)) {
        ...
        main_image = cv::Mat(HEIGHT, WIDTH, CV_8UC1, (char*)map.data);
       ...
       /* int x = 10;
        ...
        cv::rectangle(main_image, rect, cv::Scalar(255, 125, 0), 6, cv::LINE_4, 0);
*/
        //main_image = cv::Mat(frame_size, CV_8UC4, (char*)(map.data), cv::Mat::AUTO_STEP);
        copy_main_image = main_image.clone();

        cv::Rect my_interest_region(CROPPED_X, CROPPED_Y, CROPPED_WIDTH, CROPPED_HEIGHT);
